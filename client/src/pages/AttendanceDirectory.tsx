...
export default function AttendanceDirectory() {
  ...
  const loadRegister = useCallback(async () => {
    ...
    try {
      const db = getSupabase();
      const enrollment = await db.from("enrollments").select("student_id,students(id,admission_number,first_name,middle_name,last_name,gender)").eq("class_id", classId).eq("academic_year_id", yearId).eq("status", "ACTIVE");
  ...
  const visibleStudents = useMemo(() => { const q = query.trim().toLowerCase(); return students.filter(s => !q || [s.first_name,s.middle_name,s.last_name,s.admission_number,s.gender].map(v => text(v, "")).join(" ").toLowerCase().includes(q)); }, [query, students]);
