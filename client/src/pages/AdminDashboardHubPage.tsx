...
export default function AdminDashboardHubPage() {
  ...
  const metrics: Metric[] = useMemo(() => [
    ...
    { label: "Admissions queue", value: data.admissions, detail: "Pending or under review", icon: ClipboardCheck, href: "/portal/people", tone: data.admissions ? "warn" : "good" },
    ...
    { label: "Open invoices", value: data.openInvoices, detail: "Not paid or void", icon: CreditCard, href: "/portal/finance", tone: data.openInvoices ? "warn" : "good" },
  ...
  return <PortalLayout role="ADMIN"><main className="mx-auto w-full max-w-[1500px] space-y-7 px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    ...
    <section aria-label="School metrics" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((m) => <button key={m.label} type="button" onClick={() => navigate(m.href)} className="menwe-admin-metric rounded-[1.35rem] p-5 text-left transition hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-[var(--gold)]"><div className="flex items-start justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[var(--mist)] text-[var(--accent)]"><m.icon size={20} /></span><span className="text-right text-2xl font-black text-[var(--ink)]">{m.value}</span></div><p className="mt-4 text-sm font-black text-[var(--ink)]">{m.label}</p><p className="mt-1 text-xs leading-5 text-[var(--ink)]/50">{m.detail}</p></button>)}</section>
    <section className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]"><div className="menwe-card rounded-[1.75rem] p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="menwe-premium-label">Attention queue</p><h2 className="text-xl font-black text-[var(--ink)]">Work worth checking</h2></div><CheckCircle2 className="text-[var(--gold)]" size={21}/></div><div className="mt-5 grid gap-2 sm:grid-cols-2">{attention.map((item)=><button key={item.label} type="button" onClick={()=>navigate(item.href)} className="flex items-center gap-3 rounded-xl border border-[var(--ink)]/8 bg-white p-3 text-left hover:border-[var(--gold)]/40"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--mist)] text-[var(--accent)]"><item.icon size={17}/></span><span className="min-w-0 flex-1"><b className="block text-sm">{item.label}</b><small className="block text-xs text-[var(--ink)]/50">{item.detail}</small></span><strong className="text-lg">{item.value}</strong></button>)}</div></div><div className="menwe-card rounded-[1.75rem] p-5 sm:p-6"><p className="menwe-premium-label">Platform coverage</p><h2 className="text-xl font-black text-[var(--ink)]">School systems</h2><div className="mt-5 space-y-2">{capabilities.slice(0,7).map(([label,detail,Icon,href])=><button key={label} type="button" onClick={()=>navigate(href)} className="flex w-full items-center gap-3 rounded-xl p-2 text-left hover:bg-[var(--mist)]"><span className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--mist)] text-[var(--accent)]"><Icon size={16}/></span><span className="min-w-0"><b className="block text-sm">{label}</b><small className="text-xs text-[var(--ink)]/50">{detail}</small></span></button>)}</div></div></section>
  </main></PortalLayout>;
}
