...
export default function AdminDashboardHubPage() {
  ...
  return <PortalLayout role="ADMIN"><main className="mx-auto w-full max-w-[1500px] space-y-7 px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    ...
    <section aria-label="School metrics" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((m) => <button key={m.label} type="button" onClick={() => navigate(m.href)} className="menwe-admin-metric rounded-[1.35rem] p-5 text-left transition hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-[var(--gold)]"><div className="flex items-start justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[var(--mist)] text-[var(--accent)]"><m.icon size={20} /></span><span className="text-right text-2xl font-black text-[var(--ink)]">{m.value}</span></div><div className="mt-5 flex items-center gap-2"><p className="text-sm font-extrabold text-[var(--ink)]">{m.label}</p>{m.tone === "warn" ? <span aria-label="Needs attention" className="h-2 w-2 rounded-full bg-[var(--gold)]" /> : <CheckCircle2 aria-hidden="true" size={14} className="text-emerald-600" />}</div><p className="mt-1 text-xs leading-5 text-[var(--ink)]/55">{m.detail}</p></button>)}</section>

    <section className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><article className="menwe-card rounded-[1.75rem] p-6 sm:p-7"><div className="flex items-center justify-between gap-4"><div><p className="menwe-premium-label">Needs attention</p><h2 className="menwe-premium-title mt-1">Work queue</h2></div><ArrowUpRight size={18} className="text-[var(--accent)]" /></div><div className="mt-5 grid gap-2 sm:grid-cols-2">{attention.map((a) => <button key={a.label} type="button" onClick={() => navigate(a.href)} className="flex items-center gap-3 rounded-xl border border-[var(--ink)]/8 bg-white p-4 text-left transition hover:border-[var(--gold)]/35 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--gold)]"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--mist)] text-[var(--accent)]"><a.icon size={17} /></span><span className="min-w-0 flex-1"><b className="block text-sm">{a.label}</b><span className="text-xs text-[var(--ink)]/50">{a.detail}</span></span><strong className="text-lg">{a.value}</strong></button>)}</div></article><article className="menwe-card rounded-[1.75rem] p-6 sm:p-7"><p className="menwe-premium-label">School snapshot</p><h2 className="menwe-premium-title mt-1">Live operational totals</h2><div className="mt-5 grid grid-cols-2 gap-3">{[["Classes", data.classes], ["Subjects", data.subjects], ["Events", data.events], ["Exams", data.exams], ["Report cards", data.reportCards], ["Attendance", data.attendance ? `${data.attendanceRate}%` : "—"]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-[var(--mist)] p-4"><p className="text-[10px] font-black uppercase tracking-[.12em] text-[var(--ink)]/45">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>)}</div></article></section>

    <section className="menwe-card rounded-[1.75rem] p-6 sm:p-7"><div><p className="menwe-premium-label">Administrator capabilities</p><h2 className="menwe-premium-title mt-1">Manage every major school function</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--ink)]/60">Direct links to the real portal workspaces backed by the production Supabase model.</p></div><div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{capabilities.map(([label, detail, Icon, href]) => <button key={label} type="button" onClick={() => navigate(href)} className="group flex min-h-[92px] items-center gap-4 rounded-2xl border border-[var(--ink)]/8 bg-white p-4 text-left transition-all duration-300 hover:-translate-y-1 hover:border-[var(--gold)]/35 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[var(--gold)]"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[var(--ink)]/5 text-[var(--ink)]] group-hover:bg-[var(--gold)]/15 group-hover:text-[var(--gold)]"><Icon size={19} /></span><span className="min-w-0 flex-1"><b className="block text-sm">{label}</b><span className="mt-1 block text-xs leading-5 text-[var(--ink)]/50">{detail}</span></span><ArrowUpRight size={16} className="shrink-0 text-[var(--ink)]/25 group-hover:text-[var(--gold)]" /></button>)}</div></section>

    <section className="rounded-[1.75rem] border border-[var(--gold)]/20 bg-[var(--gold)]/5 p-6 sm:p-7"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 text-[var(--gold)]" size={20} /><div><h2 className="font-bold">Administration at a glance</h2><p className="mt-1 text-sm leading-6 text-[var(--ink)]/60">Use the work queue for outstanding items, quick actions for common tasks, and the capability grid when you need a complete administrative workspace.</p></div></div></section>
  </main></PortalLayout>;
}