...
export default function AdminOperationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [yearRes, termRes, studentsRes, summaryRes, attendanceRes, eventsRes, noticesRes] = await Promise.all([
        ...
        db.rpc("admin_operations_attendance_summary", { p_session_date: today }),
      ...
      for (const result of [yearRes, termRes, studentsRes, summaryRes, attendanceRes, eventsRes, noticesRes]) if (result.error) throw result.error;

      setYearName(text(yearRes.data?.name) || "Current academic year");
      setTermName(text(termRes.data?.name) || "Current term");
      const summary = (summaryRes.data?.[0] ?? {}) as Row;
      const pending = Number(summary.pending_admissions ?? 0);
      const verified = Number(summary.verified_payments ?? 0);
      const openInvoices = Number(summary.open_invoices ?? 0);
      const attendanceSummary = (attendanceRes.data?.[0] ?? {}) as Row;
      const present = Number(attendanceSummary.present_count ?? 0);
      const absent = Number(attendanceSummary.absent_count ?? 0);
      const late = Number(attendanceSummary.late_count ?? 0);
      const captured = Number(attendanceSummary.present_count ?? 0) + Number(attendanceSummary.absent_count ?? 0) + Number(attendanceSummary.late_count ?? 0);
      setAttendance({ present, absent, late, total: captured });
      setMetrics([
        ...
        { label: "Attendance today", value: String(attendanceSummary.session_count ?? 0), detail: captured ? `${captured} learner records captured` : "No learner records captured yet", icon: ClipboardCheck, tone: captured ? "good" : "warn" },
        ...
        { label: "Verified payments", value: money(verified), detail: "Verified payment value", icon: CreditCard },
        { label: "Open invoices", value: String(openInvoices), detail: "Not paid or void", icon: BarChart3, tone: openInvoices ? "warn" : "good" },
      ]);
