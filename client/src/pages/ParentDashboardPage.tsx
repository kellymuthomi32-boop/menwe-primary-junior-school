...
type Invoice = { id: string; invoice_number: string; total_amount?: number; status: string; due_on: string | null };
...
export default function ParentDashboardPage() {
    ...
    const loadLearnerData = async () => {
      try {
        ...
        const [sessionsRes, examsRes, homeworkRes, invoicesRes, announcementsRes, timetableRes] = await Promise.all([
          ...
          db.from("invoices").select("id,invoice_number,total_amount,status,due_on").eq("student_id", learner.id).order("issued_on", { ascending: false }).limit(20),
  ...
  const outstanding = invoices.reduce((sum, invoice) => { const invoiceTotal = Number(invoice.total_amount ?? 0); const paid = payments.filter(payment => payment.invoice_id === invoice.id && payment.status === "VERIFIED").reduce((p, payment) => p + Number(payment.amount), 0); return sum + Math.max(0, invoiceTotal - paid); }, 0);
    ...
    <main className="mx-auto w-full max-w-[1440px] space-y-8 px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
      ...
      {learner && <>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
          ...
          <div className="menwe-card rounded-[1.75rem] p-6 sm:p-7"><div className="flex items-center justify-between"><div><p className="menwe-premium-label">Latest</p><h2 className="mt-1 text-2xl font-extrabold">Announcements</h2></div><BarChart3 size={18} className="text-[var(--accent)]"/></div>{announcements.length ? <div className="mt-5 grid gap-3">{announcements.slice(0,3).map(item => <div key={item.id} className="rounded-2xl bg-[var(--mist)] p-4"><p className="font-extrabold">{item.title}</p><p className="mt-1 text-xs font-semibold text-[var(--ink)]/45">{shortDate(item.published_at ?? item.created_at)}</p><p className="mt-2 text-sm leading-6 text-[var(--ink)]/65">{item.body.slice(0,110)}{item.body.length > 110 ? "…" : ""}</p></div>)}</div> : <p className="mt-5 rounded-2xl bg-[var(--mist)] p-5 text-sm font-semibold text-[var(--ink)]/60">No announcements for this learner right now.</p>}</div>
        </section>

        <section className="menwe-card rounded-[1.75rem] p-6 sm:p-7"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="menwe-premium-label">Upcoming assessment</p><h2 className="mt-1 text-2xl font-extrabold">What’s next</h2></div><Link href="/portal/report-cards" className="text-sm font-extrabold text-[var(--accent)]">View results →</Link></div>{exams.length ? <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{exams.slice(0,6).map(exam => <div key={exam.id} className="rounded-2xl border border-[var(--ink)]/8 p-4"><p className="font-extrabold">{exam.name}</p><p className="mt-1 text-xs font-bold text-[var(--ink)]/50">{exam.exam_type ?? "Assessment"} · {shortDate(exam.starts_on)}</p><p className="mt-3 text-sm font-semibold text-[var(--ink)]/65">{results.some(row => row.exam_id === exam.id) ? "Result published" : "Awaiting result"}</p></div>)}</div> : <p className="mt-5 rounded-2xl bg-[var(--mist)] p-5 text-sm font-semibold text-[var(--ink)]/60">No published assessments available yet.</p>}</section>
      </>}
    </main>
    <ParentAssistant />
  </PortalLayout>;
