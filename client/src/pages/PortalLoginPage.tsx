...
import { AlertCircle, CheckCircle2, Eye, EyeOff, HelpCircle, KeyRound, LockKeyhole, ShieldCheck, UserRound } from "lucide-react";
...
export default function PortalLoginPage() {
  ...
  const finishRegistration = async (emailAddress: string, redirect: string, kind: "staff" | "parent", studentId?: string) => {
    const client = requireSupabaseClient();
    const metadata = kind === "staff"
      ? { role: "staff", full_name: fullName.trim(), staff_id: staffId.trim() }
      : { student_id: studentId, account_type: "parent_student" };
    const { data, error } = await client.auth.signUp({
      email: emailAddress,
      password,
      options: {
        data: metadata,
        ...(kind === "staff" ? { emailRedirectTo: `${window.location.origin}/portal/admin` } : {}),
      },
    });
    ...
    // With Supabase email confirmation enabled signUp succeeds without a session.
    // Try the password fallback first so disabled/optional confirmation remains seamless.
    const login = await auth.signIn(emailAddress, password);
    if (!login.error) {
      await auth.refreshProfile();
      setSuccess("Account created successfully! You are now signed in.");
      setPassword(""); setConfirmPassword("");
    ...
    if (/email not confirmed/i.test(login.error)) {
      setSuccess("Account created successfully! Please check your email inbox to confirm your account before logging in.");
      setMode("signin");
    ...
    throw new Error("Your account was created, but automatic sign-in could not be completed. Please sign in manually with your new credentials.");
  };
  ...
  const submitSignIn = async (e: FormEvent) => {
    ...
    try {
      ...
      const resolvedEmail = normalized.includes("@") ? normalized.toLowerCase() : await (async () => {
        if (tab === "family") {
  ...
  const submitRegistration = async (e: FormEvent) => {
    ...
    if (tab === "staff") {
      const name = fullName.trim(); const id = staffId.trim();
      if (!name || !id || !normalizedEmail || !password || !confirmPassword) { setMessage("Complete your full name, Staff ID / TSC Number, official email and password fields to continue."); return; }
      if (password.length < 6) { setMessage("Password must contain at least 6 characters."); return; }
      if (password !== confirmPassword) { setMessage("Passwords do not match."); return; }
      setBusy(true);
      try { await finishRegistration(normalizedEmail, "/portal/admin", "staff"); }
      catch (error) { setMessage(friendlyError(error)); }
      finally { setBusy(false); }
      return;
    }
    ...
    try {
      ...
      await finishRegistration(normalizedEmail, "/portal/dashboard", "parent", student.id);
    } catch (error) { setMessage(friendlyError(error)); }
    finally { setBusy(false); }
  };

  const requestReset = async () => {
    resetFeedback(); setResetSent(false);
  ...
  const switchMode = (next: "signin" | "register") => { setMode(next); resetFeedback(); setPassword(""); setConfirmPassword(""); setEmail(""); setFullName(""); setStaffId(""); };
  ...
  const inputClass = "mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-base outline-none focus:ring-2 focus:ring-[#D89B28]";
