...
export default function PortalLoginPage() {
  ...
  const resolveIdentifier = async (value: string) => {
    ...
    if (tab === "family") {
      const { data, error } = await client.from("students").select("parent_email,admission_number,upi_number").or(`admission_number.eq.${normalized},upi_number.eq.${normalized}`).limit(1).maybeSingle();
  ...
  const features = [
    { icon: ShieldCheck, text: "Role-aware access" },
    { icon: LockKeyhole, text: "Protected records" },
    { icon: UserRound, text: "School accounts" },
  ];
