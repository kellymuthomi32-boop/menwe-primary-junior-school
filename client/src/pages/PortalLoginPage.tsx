...
export default function PortalLoginPage() {
  ...
  const friendlyError = (error: unknown) => {
    ...
    if (/invalid login credentials/i.test(text)) return "Incorrect email or password. Please double-check your credentials.";
    if (/email not confirmed/i.test(text)) return "Your email address has not been confirmed yet. Please check your inbox or toggle off email confirmation in Supabase.";
    if (/rate limit|too many requests|429|network|fetch|timeout|failed to/i.test(text)) return "Connection issue. Please check your network and try again.";
  ...
  const submitRegistration = async (event: FormEvent) => {
    ...
    if (!normalizedEmail || !password || !confirmPassword) {
      setMessage("Complete all registration fields to continue.");
      return;
    }
      ...
      <section className="bg-[#F8F9FA]">
        <div className="mx-auto grid min-h-[calc(100vh-10rem)] max-w-6xl items-center gap-8 px-5 py-16 lg:grid-cols-[1fr_.8fr] lg:px-8">
          <div>
            ...
            <p className="mt-6 max-w-xl text-base leading-7 text-slate-600">Parents, students and staff receive access according to their authorised school role.</p>
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {features.map(({ icon: Icon, text }) => (
                <div key={text} className="rounded-xl bg-white p-4 shadow-sm">
                  <Icon className="text-[#D89B28]" size={20} />
                  <p className="mt-3 text-xs font-bold text-[#061229]">{text}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md dark:border-slate-800 dark:bg-slate-900 sm:p-8">
            <div className="grid grid-cols-2 rounded-xl bg-[#F8F9FA] p-1">
              <button type="button" onClick={() => switchTab("family")} className={`min-h-12 rounded-lg px-3 py-3 text-xs font-bold ${tab === "family" ? "bg-white text-[#061229] shadow-sm" : "text-slate-500"}`}>Parent / Student</button>
              <button type="button" onClick={() => switchTab("staff")} className={`min-h-12 rounded-lg px-3 py-3 text-xs font-bold ${tab === "staff" ? "bg-white text-[#061229] shadow-sm" : "text-slate-500"}`}>Staff / Teacher</button>
            </div>
            {successBanner}
            {mode === "signin" ? (
              <form onSubmit={submitSignIn} className="mt-7">
                <label className="text-sm font-bold text-[#061229] dark:text-white">
                  {tab === "family" ? "Admission Number / UPI / Email" : "Staff ID / Email"}
                  <input required={!magicLinkMode} value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder={tab === "family" ? "Admission number, UPI or registered email" : "Staff ID or official registered email"} className={inputClass} />
                </label>
                {!magicLinkMode && (
                  <label className="mt-5 block text-sm font-bold text-[#061229] dark:text-white">
                    Password
                    <div className="relative mt-2">
                      <input required type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" className="w-full min-h-12 rounded-xl border border-slate-200 bg-white px-4 py-3 pr-12 text-base outline-none focus:ring-2 focus:ring-[#D89B28]" />
                      <button type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword(v => !v)} className="absolute right-2 top-1/2 min-h-10 min-w-10 -translate-y-1/2 text-slate-500">
                ...
                {alert}
                <button disabled={busy} className="mt-6 min-h-12 w-full rounded-xl bg-[#061229] px-5 py-3.5 text-base font-extrabold text-white transition active:scale-[.98] disabled:opacity-60">
                  {busy ? (magicLinkMode ? "Sending magic link…" : "Signing in…") : (magicLinkMode ? "Send magic link" : "Sign in securely")}
                </button>
                {tab === "family" && (
                  <button type="button" onClick={() => { clearFeedback(); setMagicLinkMode(v => !v); }} className="mt-4 min-h-12 w-full text-sm font-bold text-[#D89B28] hover:underline">
                    {magicLinkMode ? "Back to password sign in" : "Sign in with Email Link (Magic Link)"}
                  </button>
                )}
                <div className="mt-3 text-center">
                  <button type="button" onClick={() => switchMode("register")} className="min-h-12 px-2 text-sm font-bold text-[#D89B28] hover:underline">
                    {tab === "staff" ? "New teacher or staff member? Create an account here" : "Don't have an account yet? Register here"}
                  </button>
                </div>
              </form>
            ) : (
              <form onSubmit={submitRegistration} className="mt-7">
                <h2 className="text-2xl font-bold text-[#061229] dark:text-white">{tab === "staff" ? "Staff Registration" : "Parent & Student Registration"}</h2>
                ...
                <label className="mt-5 block text-sm font-bold text-[#061229] dark:text-white">{tab === "staff" ? "Official Email Address" : "Parent / Guardian Email"}<input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={tab === "staff" ? "teacher@example.com" : "parent@example.com"} className={inputClass} /></label>
                {tab === "family" && (
                  <label className="mt-5 block text-sm font-bold text-[#061229] dark:text-white">Learner Admission No. or UPI Number<input required value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder="Admission number or UPI" className={inputClass} /></label>
                )}
                <label className="mt-5 block text-sm font-bold text-[#061229] dark:text-white">Password<input required minLength={6} type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} className={inputClass} /></label>
                <label className="mt-5 block text-sm font-bold text-[#061229] dark:text-white">Confirm Password<input required minLength={6} type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputClass} /></label>
                {alert}
                <button disabled={busy} className="mt-6 min-h-12 w-full rounded-xl bg-[#061229] px-5 py-3.5 text-base font-extrabold text-white transition active:scale-[.98] disabled:opacity-60">
                  {busy ? "Creating account…" : tab === "staff" ? "Create staff account" : "Create account"}
                </button>
                <p className="mt-5 text-center text-sm text-slate-500">Already registered? <button type="button" onClick={() => switchMode("signin")} className="min-h-12 px-1 font-bold text-[#D89B28] hover:underline">Sign in here</button></p>
              </form>
            )}
            <div className="mt-6 rounded-xl bg-[#F8F9FA] p-4 text-xs leading-5 text-slate-600">
              <div className="flex gap-2">
                <HelpCircle size={16} className="shrink-0 text-[#D89B28]" />
                <p><strong className="text-[#061229]">Having trouble?</strong> Contact the admin office at 0142550882 or visit the ICT desk.</p>
              </div>
            </div>
          </div>
        </div>
