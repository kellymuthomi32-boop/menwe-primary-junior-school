...
export default function PortalLoginPage() {
  ...
  const submitSignIn = async (event: FormEvent) => {
    ...
    try {
      ...
      go(tab === "staff" ? "/portal/admin" : "/portal/dashboard");
  ...
  const sendMagicLink = async () => {
    ...
    if (!emailAddress || !emailAddress.includes("@")) { setMessage("Enter your registered email address to receive a magic link."); return; }
    setBusy(true);
    try {
      const client = requireSupabaseClient();
      const { error } = await client.auth.signInWithOtp({ email: emailAddress, options: { emailRedirectTo: `${window.location.origin}/portal/dashboard`, shouldCreateUser: false } });
  ...
  const submitRegistration = async (event: FormEvent) => {
    ...
    try {
      ...
      if (tab === "staff") {
        ...
        if (!name || !id) { setMessage("Complete your full name and Staff ID / TSC Number."); return; }
        metadata = { role: "staff", full_name: name, staff_id: id };
      ...
      const client = requireSupabaseClient();
      const { data, error } = await client.auth.signUp({ email: normalizedEmail, password, options: { data: metadata, emailRedirectTo: `${window.location.origin}${redirect}` } });
      if (error) { setMessage(friendlyError(error)); return; }
      if (data.session) {
        await auth.refreshProfile();
      ...
      if (/email not confirmed/i.test(login.error)) {
        ...
        setMode("signin"); setIdentifier(normalizedEmail); setPassword(""); setConfirmPassword("");
      } else {
  ...
  const switchTab = (next: "family" | "staff") => { setTab(next); setMode("signin"); setMagicLinkMode(false); clearFeedback(); setIdentifier(""); setPassword(""); };
  const switchMode = (next: "signin" | "register") => { setMode(next); setMagicLinkMode(false); clearFeedback(); setPassword(""); setConfirmPassword(""); };
