...
const slides = [
  {
    src: "https://commons.wikimedia.org/wiki/Special:Redirect/file/AHS_Uniform.jpg",
    alt: "Kenyan school learners in uniform; Menwe Primary learners wear sky-blue dresses with white-and-blue checked collars, while boys wear navy-blue shorts with sky-blue shirts",
    caption: "Primary learners · Sky-blue and navy uniform",
  },
  {
    src: "https://commons.wikimedia.org/wiki/Special:Redirect/file/African%20girl%20in%20her%20school%20uniform%20riding%20her%20bicycle%20barefooted.jpg",
    alt: "African school learner in uniform; Menwe primary uniform visual identity uses sky-blue, white-and-blue checks, and navy-blue shorts",
    caption: "Primary school · Everyday learner life",
  },
  {
    src: "https://commons.wikimedia.org/wiki/Special:Redirect/file/Bishop%20Gatimu%20Girls%20Scouts.jpg",
    alt: "Kenyan junior school learners; Menwe Junior learners wear maroon sweaters with navy-blue shirts or blouses",
    caption: "Junior learners · Maroon and navy uniform",
  },
  {
    src: "https://commons.wikimedia.org/wiki/Special:Redirect/file/Kids%20receiving%20ball%20-%20Winneba%20School-Ghana%20Keri%20Oberly.jpg",
    alt: "African school sports scene with learners handling a rugby-style ball, representing Menwe's rugby and wider sports culture",
    caption: "Sports culture · Rugby, teamwork and fitness",
  },
  {
    src: "https://commons.wikimedia.org/wiki/Special:Redirect/file/A%20playful%20African%20school%20boy.jpg",
    alt: "African school learner enjoying school life; Menwe celebrates confident, active learners across classroom and co-curricular experiences",
    caption: "School life · Confidence beyond the classroom",
  },
];
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center overflow-hidden rounded-[2rem]">
        <div className="flex w-full h-full items-center justify-center gap-2 sm:gap-3">
          {slides.map((slide, index) => (
            ...
            >
              <img src={slide.src} alt={slide.alt} loading={index === 0 ? "eager" : "lazy"} className="h-full w-full object-cover" />
              <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#061229]/90 via-[#061229]/35 to-transparent px-3 pb-3 pt-10 text-left text-[11px] sm:text-xs font-semibold text-white leading-tight">
                {slide.caption}
              </span>
            </button>
          ))}
        </div>
        <div className="absolute left-4 top-4 z-20 max-w-[80%] rounded-full border border-white/15 bg-[#061229]/75 px-3 py-1.5 text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-white backdrop-blur-sm">
          {activeSlide.caption}
        </div>
    ...
    <div className="relative z-20 mx-auto mt-6 w-full max-w-7xl px-0" aria-label="Explore Menwe">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 w-full">
        {homeNavigation.map(([title, path, text, Icon]) => (
          <button key={path} type="button" onClick={() => navigate(path)} aria-label={`Open ${title}`} className="group w-full min-w-0 min-h-12 rounded-2xl p-6 bg-white border border-slate-200/80 shadow-sm text-left hover:border-[#D89B28]/50 hover:shadow-xl transition-all duration-300 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D89B28]">
            <span className="w-12 h-12 rounded-xl bg-amber-50 text-[#D89B28] flex items-center justify-center font-bold text-lg mb-4"><Icon size={21} /></span>
            <span className="flex items-center justify-between gap-4"><span className="min-w-0 text-lg sm:text-xl font-bold text-slate-900 break-words">{title}</span><span className="w-10 h-10 shrink-0 rounded-full flex items-center justify-center bg-slate-50 text-slate-500 transition-transform group-hover:translate-x-1"><ArrowRight size={17} /></span></span>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-[#061229] via-[#0b1d3a] to-[#061229] px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-14" aria-labelledby="hero-title">
          ...
          <div className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-8 lg:grid-cols-12 lg:gap-10">
            <div className="min-w-0 lg:col-span-5">
              ...
              <div className="mt-7 flex flex-col sm:flex-row gap-3 w-full">
                <button type="button" onClick={() => navigate("/admissions")} className="w-full sm:w-auto min-h-12 bg-[#D89B28] hover:bg-[#c28920] text-[#061229] font-bold px-6 py-3.5 rounded-xl transition-all shadow-lg active:scale-[0.98] inline-flex items-center justify-center gap-2">Apply for 2026 Admissions <ArrowRight size={17} /></button>
                <button type="button" onClick={() => navigate("/login")} className="w-full sm:w-auto min-h-12 border border-white/20 hover:bg-white/10 text-white font-medium px-6 py-3.5 rounded-xl transition-all backdrop-blur-sm inline-flex items-center justify-center gap-2 active:scale-[0.98]">Explore Student Portal <UsersRound size={17} /></button>
              </div>
            </div>
            <div className="min-w-0 lg:col-span-7"><HeroGallery /></div>
          </div>
          <HomeNavigationCards />
          <div className="relative z-20 mx-auto mt-6 w-full max-w-7xl rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6 backdrop-blur-md">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[["1961", "Founded"], ["23+", "TSC & BOM Faculty"], ["100%", "CBC Pathway Integration"], ["0142550882", "Direct Admissions Hotline"]].map(([value, label]) => <div key={label} className="min-w-0 text-center sm:text-left"><div className="text-xl sm:text-2xl font-extrabold tracking-tight text-white break-words">{value}</div><div className="mt-1 text-xs sm:text-sm text-white/55 leading-relaxed">{label}</div></div>)}
            </div>
          </div>
        </section>
