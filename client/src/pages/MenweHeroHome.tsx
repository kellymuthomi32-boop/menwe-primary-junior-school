...
const slides = [
  { src: "/hero-learning.svg", alt: "Menwe learning pathway illustration", caption: "Primary learners · Strong learning foundations" },
  { src: "/hero-community.svg", alt: "Menwe school community illustration", caption: "Primary school · Everyday learner life" },
  { src: "/hero-arts.svg", alt: "Menwe arts and talent illustration", caption: "Junior learners · Creativity and confidence" },
  { src: "/hero-sport.svg", alt: "Menwe school sports illustration", caption: "Sports culture · Teamwork and fitness" },
  { src: "/hero-science.svg", alt: "Menwe science and innovation illustration", caption: "School life · Discovery beyond the classroom" },
];
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center overflow-hidden rounded-[2rem]">
        <div className="flex w-full h-full items-center justify-center gap-2 sm:gap-3">
          {slides.map((slide, index) => (
            <button key={slide.src} type="button" onClick={() => setActive(index)} aria-label={`Show image ${index + 1}: ${slide.caption}`} className={`relative h-[78%] flex-1 min-w-0 overflow-hidden rounded-2xl border border-white/10 shadow-2xl transition-all duration-500 ${index === active ? "scale-105 z-10 opacity-100" : "opacity-65"}`}>
        ...
        <div className="absolute bottom-4 left-1/2 z-20 -translate-x-1/2 flex gap-1.5" aria-label="Gallery pagination">
          {slides.map((slide, index) => <button key={slide.src} type="button" onClick={() => setActive(index)} aria-label={`Go to image ${index + 1}`} className={`h-1.5 rounded-full transition-all ${index === active ? "w-7 bg-[#D89B28]" : "w-1.5 bg-white/50"}`} />)}
        </div>
      </div>
    </div>
  );
