  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const subjectIds = [...new Set((a.data ?? []).map(x => String(x.subject_id)))];
        if (!classIds.length) { if (active) { setAssignments([]); setRoles(r.data ?? []); setClasses([]); setSubjects([]); setLearners([]); } return; }
        // Fetch the exact subjects referenced by the teacher assignments rather than
        // relying on a global ACTIVE-subject filter. This keeps an assignment visible
        // even if an administrator changes a subject's status later.
        const [c, s, e] = await Promise.all([
          db.from("classes").select("id,name,code,level,academic_year_id,status").in("id", classIds),
          subjectIds.length ? db.from("subjects").select("id,code,name,status").in("id", subjectIds) : Promise.resolve({ data: [], error: null } as any),
