...
export default function TeacherDashboardPage() {
  ...
  return <PortalLayout role="TEACHER"><main className="mx-auto w-full max-w-[1440px] space-y-8 px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    ...
    <section id="my-learners" className="menwe-card rounded-[1.75rem] p-6 sm:p-8"><p className="menwe-premium-label">My Learners</p><h2 className="menwe-premium-title mt-1">Filtered student roster</h2><p className="mt-2 text-sm text-[var(--ink)]/60">Only active learners in your assigned classes are listed.</p><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr className="border-b border-[var(--ink)]/10 text-xs font-bold uppercase tracking-[.1em] text-[var(--ink)]/45"><th className="p-3">Learner</th><th className="p-3">Admission</th><th className="p-3">Class</th></tr></thead><tbody>{learners.map(s=>{const cls=classes.find(c=>String(c.id)===String(s.class_id));return <tr key={String(s.id)} className="border-b border-[var(--ink)]/7"><td className="p-3 font-semibold">{[s.first_name,s.middle_name,s.last_name].filter(Boolean).join(" ")}</td><td className="p-3">{s.admission_number}</td><td className="p-3">{cls?.name ?? "—"}</td></tr>})}</tbody></table>{!loading&&!learners.length&&<p className="py-10 text-center text-sm text-[var(--ink)]/55">No active learners are currently enrolled in your assigned classes.</p>}</div></section>
