...
const modules = [
  ...
  { title: "Exams & results", href: "/portal/exams", icon: FileText, text: "Record and review authorised assessment results." },
...
export default function TeacherDashboardPage() {
  ...
  return <PortalLayout role="TEACHER"><main className="mx-auto w-full max-w-[1440px] space-y-8 px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    ...
    <section aria-label="Teacher modules"><div className="mb-5"><p className="menwe-premium-label">Teaching tools</p><h2 className="menwe-premium-title mt-1">Everything you need, without the clutter.</h2><p className="menwe-premium-copy mt-2 max-w-2xl">Clear actions, strong hierarchy and real school records make the workspace faster to use during the school day.</p></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">{modules.map(({ title, href, icon: Icon, text }) => <Link key={href} href={href} className="menwe-card menwe-interactive group rounded-[1.35rem] p-6 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--gold)]"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--mist)] text-[var(--accent)]"><Icon size={22}/></span><h2 className="mt-5 text-lg font-extrabold text-[var(--ink)]">{title}</h2><p className="mt-2 text-sm leading-6 text-[var(--ink)]/65">{text}</p><span className="mt-6 inline-flex text-sm font-extrabold text-[var(--accent)] transition-transform group-hover:translate-x-1">Open workspace <span className="ml-1.5">→</span></span></Link>)}</div></section>
