...
export default function PortalParentPage(){
  ...
  return <><div className="mx-auto flex w-full max-w-7xl min-w-0 box-border justify-end px-4 pt-4 sm:px-6 lg:px-8"><ReportCardButton userId={user?.id}/></div>{upis.length>0&&<div className="mx-auto w-full max-w-7xl min-w-0 box-border px-4 pt-4 sm:px-6 lg:px-8"><div className="grid grid-cols-1 gap-5">{upis.map(upi=><CbcPortfolioView key={upi} upi={upi}/>)}</div></div>}<PortalDashboardPage/></>;
}
