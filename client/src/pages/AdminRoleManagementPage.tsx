...
export default function AdminRoleManagementPage() {
  ...
  const save = async (row: StaffRow) => {
    ...
    if (profile?.role === "ADMIN" && next === "SUPER_ADMIN") {
      setMessage("Only a Super Admin can assign the Super Admin role.");
      return;
    }
    setSaving(row.id);
    setMessage(null);
