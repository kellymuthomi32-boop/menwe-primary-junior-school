...
export default function AdminRoleManagementPage() {
  ...
  return <main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
    ...
    <section className="menwe-card overflow-hidden rounded-[1.75rem]">
      <div className="flex items-center gap-3 border-b border-[var(--ink)]/8 px-5 py-4 sm:px-6"><Users size={18} className="text-[var(--gold)]"/><div><h2 className="font-serif text-xl font-semibold">School accounts</h2><p className="text-xs text-[var(--ink)]/50">Changing a role takes effect on the user’s next authenticated profile refresh.</p></div></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="border-b border-[var(--ink)]/10 text-[10px] font-black uppercase tracking-[.13em] text-[var(--ink)]/45"><th className="px-5 py-3">Name</th><th className="px-5 py-3">Email</th><th className="px-5 py-3">Phone</th><th className="px-5 py-3">Role</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Action</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b border-[var(--ink)]/7 last:border-0 hover:bg-[var(--gold)]/[.035]"><td className="px-5 py-4 font-semibold">{row.full_name || "Unnamed user"}</td><td className="px-5 py-4">{row.email || "—"}</td><td className="px-5 py-4">{row.phone || "—"}</td><td className="px-5 py-4"><select value={draft[row.id] || row.role} onChange={(e) => setDraft((current) => ({ ...current, [row.id]: e.target.value as AppRole }))} className="min-h-11 rounded-xl border border-[var(--ink)]/12 bg-white px-3 text-sm font-semibold outline-none focus:border-[var(--gold)]">{roles.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></td><td className="px-5 py-4"><span className="rounded-full bg-[var(--gold)]/10 px-2.5 py-1 text-[10px] font-black uppercase text-[var(--gold)]">{row.status}</span></td><td className="px-5 py-4"><button type="button" disabled={saving === row.id || draft[row.id] === row.role} onClick={() => void save(row)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--ink)] px-4 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">{saving === row.id ? <Loader2 className="animate-spin" size={16}/> : <Save size={16}/>} {saving === row.id ? "Saving…" : "Save role"}</button></td></tr>)}</tbody></table>{rows.length === 0 && <div className="py-16 text-center text-sm text-[var(--ink)]/50">No school accounts found.</div>}</div>
    </section>
  </main>;
}
