...
function TeacherContent() {
  ...
  const generateAiRemarks = async () => {
    ...
    try {
      ...
      if (!baseUrl) throw new Error("Supabase is not configured for this deployment.");
      const response = await fetch(`${baseUrl}/functions/v1/cbc-remarks`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ grade_level: grade, learning_area: area, rubric_score: score, current_remarks: remarks.trim() }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof payload.error === "string" ? payload.error : "The AI assistant could not generate remarks.");
      setRemarks(typeof payload.remark === "string" ? payload.remark : "");
      setMessage("AI draft generated. Review it and edit it before saving the assessment.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to generate AI remarks right now."); }
    finally { setGeneratingRemarks(false); }
  };
  ...
  return <PortalLayout role={auth.profile!.role}><main className="mx-auto w-full max-w-7xl box-border min-w-0 overflow-hidden px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
    ...
    {tab === "gradebook" ? <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="text-2xl font-bold text-[#061229]">Record CBC learning evidence</h2><div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2"><label className="text-sm font-bold">Learner UPI<input value={upi} onChange={e => setUpi(e.target.value)} className="mt-2 box-border min-h-12 w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-4" placeholder="UPI number"/></label><label className="text-sm font-bold">Grade<select value={grade} onChange={e => setGrade(e.target.value)} className="mt-2 box-border min-h-12 w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-4">{["PP1","PP2",...Array.from({ length: 9 }, (_, i) => `Grade ${i + 1}`)].map(x => <option key={x}>{x}</option>)}</select></label><label className="text-sm font-bold">Learning Area<select value={area} onChange={e => setArea(e.target.value)} className="mt-2 box-border min-h-12 w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-4">{["Mathematics","English","Kiswahili","Integrated Science","Social Studies","Agriculture","Creative Arts","Pre-Technical Studies","ICT","Health Education","Sports Science"].map(x => <option key={x}>{x}</option>)}</select></label><label className="text-sm font-bold">Rubric<select value={score} onChange={e => setScore(e.target.value)} className="mt-2 box-border min-h-12 w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-4"><option>EE</option><option>ME</option><option>AE</option><option>BE</option></select></label></div><label className="mt-4 block text-sm font-bold">Teacher remarks<textarea value={remarks} onChange={e => setRemarks(e.target.value)} className="mt-2 box-border min-h-28 w-full min-w-0 max-w-full rounded-xl border border-slate-200 p-4" placeholder="Evidence or support notes"/></label><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"><button disabled={generatingRemarks || busy} onClick={() => void generateAiRemarks()} className="min-h-12 w-full rounded-xl border border-[#D89B28]/40 bg-amber-50 px-5 font-bold text-[#8a6117] transition active:scale-[.98] disabled:opacity-60">{generatingRemarks ? <><Loader2 className="mr-2 inline animate-spin" size={16}/>Drafting…</> : <><Sparkles className="mr-2 inline" size={16}/>Draft with CBC AI</>}</button><button disabled={busy || generatingRemarks} onClick={() => void save()} className="min-h-12 w-full rounded-xl bg-[#061229] px-5 font-bold text-white transition active:scale-[.98] disabled:opacity-60">{busy ? <><Loader2 className="mr-2 inline animate-spin" size={16}/>Saving…</> : <><Save className="mr-2 inline" size={16}/>Save CBC assessment</>}</button></div><p className="mt-3 text-xs text-slate-500">AI output is a draft only. Confirm accuracy and add your own professional observations before saving.</p></section> : <section className="mt-6"><AttendanceDirectory/></section>}
  </main></PortalLayout>;
}
