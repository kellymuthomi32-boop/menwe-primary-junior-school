  ...
  return <PortalLayout role={profile.role}><main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8"><div className="grid gap-6">
    {admin && <section className="menwe-card rounded-[1.75rem] border border-[var(--gold)]/30 p-5 sm:p-7">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div><p className="menwe-premium-label">Academic Management</p><h1 className="mt-1 font-serif text-3xl font-semibold">Academic Setup</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--ink)]/60">Manage academic years, terms, classes and subjects here. Marks entry has its own dedicated workspace.</p></div>
        <button type="button" onClick={() => setLocation("/portal/marks-management")} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--ink)] px-5 text-sm font-bold text-white shadow-sm hover:opacity-90"><ClipboardList size={18}/>Open Marks Management</button>
      </div>
      <div className="mt-5 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-5">
        {["Academic year", "Term", "Class", "Examination", "Subject → marks workspace"].map((step, index) => <div key={step} className="rounded-xl border border-[var(--ink)]/8 bg-[var(--mist)]/50 p-3"><span className="text-xs font-black text-[var(--gold)]">{index + 1}</span><p className="mt-1 font-semibold">{step}</p></div>)}
      </div>
    </section>}
