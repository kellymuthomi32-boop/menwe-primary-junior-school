...
function getBand(c?: Row) {
  ...
  if (/LOWER|GRADE 1|GRADE 2|GRADE 3|PRIMARY 1|PRIMARY 2|PRIMARY 3/.test(raw)) return "LOWER_PRIMARY" as const;
  if (/UPPER|GRADE 4|GRADE 5|GRADE 6|PRIMARY 4|PRIMARY 5|PRIMARY 6/.test(raw)) return "UPPER_PRIMARY" as const;
  if (/JUNIOR|JSS|JUNIOR SCHOOL|GRADE 7|GRADE 8|GRADE 9/.test(raw)) return "JUNIOR_SCHOOL" as const;
  return "JUNIOR_SCHOOL" as const;
}
...
export default function ClassMarksheetV2() {
  ...
  const [years, setYears] = useState<Row[]>([]), [terms, setTerms] = useState<Row[]>([]), [classes, setClasses] = useState<Row[]>([]), [subjects, setSubjects] = useState<Row[]>([]), [rows, setRows] = useState<Row[]>([]);
  ...
  useEffect(() => { void (async () => {
    ...
    try {
      ...
      setYears(y.data ?? []); setTerms(t.data ?? []); setClasses(c.data ?? []); setSubjects(s.data ?? []);
  ...
  const generate = async () => {
    ...
    try {
      ...
      const [st, rr] = await Promise.all([
        ...
        db.from("exam_results").select("student_id,subject_id,score,maximum_score,grade,exams!inner(id,term_id,class_id,name,exam_type)").eq("exams.term_id", termId).eq("exams.class_id", classId).in("student_id", ids)
      ]);
      ...
      if (!results.length) setMessage("No persisted assessment results exist for this class and term. Empty marks are shown.");
