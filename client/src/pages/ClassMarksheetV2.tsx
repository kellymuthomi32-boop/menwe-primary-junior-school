...
function casComposite(row: Row, subjects: Subject[]) {
  ...
  const values = components.map(subject => {
    ...
    const normalized = results.map((r: Row) => {
...
export default function ClassMarksheetV2() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const ids = [...new Set(enrollmentRows.map(x => str(x.student_id)))];
      if (!ids.length) { setRows([]); setMessage("No active learners are enrolled in this class. Check the learner status and class assignment."); return; }
  ...
  const resultMark = (r: Row, subjectId: string) => {
    ...
    const vals = (r.results ?? []).filter((x: Row) => str(x.subject_id) === subjectId || (!!code && str(x.subjects?.code).toUpperCase() === code)).map((x: Row) => num(x.score)).filter((x: number | null): x is number => x != null);
    return vals.length ? fmt(vals[vals.length - 1]) : "";
  };
  const resultLevel = (r: Row, subjectId: string) => getResultForSubject(r, subjectId).map((x: Row) => str(x.grade)).filter(Boolean).filter((v: string, i: number, a: string[]) => a.indexOf(v) === i).join(" / ");
  const compositeLevel = (r: Row) => {
    const existing = getResultForSubject(r, str(subjects.find(s => str(s.code).toUpperCase() === CAS_PARENT)?.id ?? ""));
    const stored = existing.map((x: Row) => str(x.grade)).filter(Boolean).pop();
    return stored || "";
  };
  const subjectTotals = useMemo(() => displaySubjects.map(s => {
    ...
    const vals = rows.flatMap((r: Row) => getResultForSubject(r, str(s.id)).map((x: Row) => num(x.score)).filter((x: number | null): x is number => x != null));
