...
export default function ReportCardsDirectory() {
  ...
  const resetReport = () => { setMessage(null); setReport(null); setItems([]); setResults([]); setAttendance([]); setAnnualRows([]); setSubjects([]); };
  ...
  const generateTermReport = async () => {
    ...
    try {
      const db = getSupabase();
      const [en, r, a, existing, sub] = await Promise.all([
        db.from("enrollments").select("class_id").eq("student_id", studentId).eq("academic_year_id", yearId).eq("status", "ACTIVE").limit(1),
        db.from("exam_results").select("id,exam_id,subject_id,score,maximum_score,grade,teacher_comment,exams!inner(id,term_id,class_id,name,exam_type,starts_on,ends_on,status),subjects(id,name,code)").eq("student_id", studentId).eq("exams.term_id", termId),
      ...
      for (const q of [en, r, a, existing, sub]) if (q.error) throw q.error;
      const resultRows = r.data ?? [];
      setResults(resultRows); setAttendance(a.data ?? []); setSubjects(sub.data ?? []);
      const classId = String(en.data?.[0]?.class_id ?? existing.data?.class_id ?? "");
      if (existing.data) {
        const ri = await db.from("report_card_items").select("id,report_card_id,subject_id,score,maximum_score,grade,subjects(id,name,code)").eq("report_card_id", existing.data.id);
      ...
      else if (!resultRows.length) setMessage("Report card opened. No marks have been recorded for this learner and term yet.");
  ...
  const generateAnnualReport = async () => {
    ...
    try {
      ...
      const [en, examsQ, attQ, subQ] = await Promise.all([
        db.from("enrollments").select("class_id").eq("student_id", studentId).eq("academic_year_id", yearId).eq("status", "ACTIVE").limit(1),
      ...
      for (const q of [en, examsQ, attQ, subQ]) if (q.error) throw q.error;
      setSubjects(subQ.data ?? []); setAttendance(attQ.data ?? []);
      const exams = examsQ.data ?? [];
      const examIds = exams.map((x: any) => x.id);
      if (!examIds.length) { setMessage("End-of-year report opened. No published Opener or End Term examinations are available yet."); return; }
      ...
      const annual = [...bySubject.values()].map(row => { const opener = assessmentValue(row.openerRows), endTerm = assessmentValue(row.endRows); const average = opener != null && endTerm != null ? (opener + endTerm) / 2 : opener ?? endTerm; return { ...row, opener, endTerm, average, performance: performanceLevel(average) }; }).filter(row => row.opener != null || row.endTerm != null);
      setAnnualRows(annual);
      if (!annual.length) setMessage("End-of-year report opened. No Opener or End Term learner marks have been recorded yet.");
      else setMessage("End-of-year learner report prepared from Opener and End Term results.");
  ...
  const termSubjects = useMemo(() => {
    ...
    for (const s of subjects) map.set(String(s.id), { subject_id: s.id, subject: s.name, code: s.code, score: null, maximum_score: null, grade: null, teacher_comment: null });
    for (const r of results) map.set(String(r.subject_id), { ...map.get(String(r.subject_id)), ...r, subject: r.subjects?.name ?? map.get(String(r.subject_id))?.subject ?? r.subject_id });
    for (const r of items) map.set(String(r.subject_id), { ...map.get(String(r.subject_id)), ...r, subject: r.subjects?.name ?? map.get(String(r.subject_id))?.subject ?? r.subject_id });
    return [...map.values()];
  ...
  return <PortalLayout role={profile?.role ?? "PARENT"}><main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    <section className="menwe-card rounded-[1.75rem] p-5 sm:p-7 print:p-0">
      ...
      {hasReport && selected && <article className="report-print mt-8 rounded-2xl border border-[var(--ink)]/15 bg-white p-6 print:border-0 print:p-0">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-[var(--ink)]/5 p-4"><span className="text-xs font-bold uppercase text-[var(--ink)]/50">{mode === "annual" ? "Year average" : "Average"}</span><p className="mt-1 text-2xl font-black">{fmt(mode === "annual" ? annualAverage : termAverage, "%")}</p></div><div className="rounded-xl bg-[var(--ink)]/5 p-4"><span className="text-xs font-bold uppercase text-[var(--ink)]/50">Attendance</span><p className="mt-1 text-2xl font-black">{fmt(attendancePct, "%")}</p></div><div className="rounded-xl bg-[var(--ink)]/5 p-4"><span className="text-xs font-bold uppercase text-[var(--ink)]/50">Subjects</span><p className="mt-1 text-2xl font-black">{mode === "annual" ? annualRows.length || subjects.length : termSubjects.length}</p></div></div>
        {mode === "annual" ? <><div className="mt-7 overflow-x-auto"><table className="w-full min-w-[700px] border-collapse text-sm"><thead><tr className="bg-[var(--ink)] text-white"><th className="p-3 text-left">Learning Area</th><th className="p-3 text-center">Opener</th><th className="p-3 text-center">End Term</th><th className="p-3 text-center">Average</th><th className="p-3 text-left">Performance Level</th></tr></thead><tbody>{(annualRows.length ? annualRows : subjects.map(s => ({ subject_id: s.id, subject: s.name, opener: null, endTerm: null, average: null, performance: "—" }))).map((r, i) => <tr key={text(r.subject_id, String(i))} className="border-b border-[var(--ink)]/10"><td className="p-3 font-bold">{text(r.subject)}</td><td className="p-3 text-center">{fmt(r.opener, "%")}</td><td className="p-3 text-center">{fmt(r.endTerm, "%")}</td><td className="p-3 text-center font-black">{fmt(r.average, "%")}</td><td className="p-3 font-semibold">{text(r.performance)}</td></tr>)}</tbody><tfoot><tr className="border-t-2 border-[var(--ink)]/25 bg-[var(--gold)]/10 font-black"><td className="p-3 uppercase">Overall average</td><td className="p-3 text-center">{fmt(annualOpener, "%")}</td><td className="p-3 text-center">{fmt(annualEnd, "%")}</td><td className="p-3 text-center">{fmt(annualAverage, "%")}</td><td className="p-3">{performanceLevel(annualAverage)}</td></tr></tfoot></table></div>{!annualRows.length && <div className="mt-5 rounded-xl border border-dashed border-[var(--gold)]/50 bg-[var(--gold)]/10 px-4 py-3 text-sm font-semibold">No marks recorded yet. The report card is ready and will populate when Opener or End Term results are entered.</div>}</> : <><div className="mt-7 overflow-x-auto"><table className="w-full min-w-[680px] border-collapse text-sm"><thead><tr className="border-b-2"><th className="p-3 text-left">Learning Area</th><th className="p-3 text-left">Score</th><th className="p-3 text-left">Maximum</th><th className="p-3 text-left">Grade</th><th className="p-3 text-left">Teacher comment</th></tr></thead><tbody>{termSubjects.map((r, i) => <tr key={text(r.id, `${text(r.subject_id)}-${i}`)} className="border-b"><td className="p-3 font-semibold">{text(r.subject)}</td><td className="p-3">{r.score == null ? "—" : text(r.score)}</td><td className="p-3">{r.maximum_score == null ? "—" : text(r.maximum_score)}</td><td className="p-3 font-bold">{text(r.grade)}</td><td className="p-3">{text(r.teacher_comment, "")}</td></tr>)}</tbody></table></div>{!results.length && !items.length && <div className="mt-5 rounded-xl border border-dashed border-[var(--gold)]/50 bg-[var(--gold)]/10 px-4 py-3 text-sm font-semibold">No marks recorded yet. This learner's report card is visible and ready for results to be entered.</div>}</>}
