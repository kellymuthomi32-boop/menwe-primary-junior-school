import { BookOpen, ExternalLink, FileText, Link2, Loader2, PlayCircle, Plus, Search, Sparkles, Video, X } from "lucide-react";
...
export default function LearningMaterialsDirectory() {
  ...
  const [form, setForm] = useState({ title: "", description: "", material_type: "LINK", resource_url: "", class_id: "", subject_id: "" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (teacher) {
        ...
        if (!tr.data) throw new Error("Your teacher record is not linked yet. Ask an administrator to complete your staff profile.");
      ...
      const classIds = (assignments?.data ?? []).map(x => String(x.class_id));
      const subjectIds = (assignments?.data ?? []).map(x => String(x.subject_id));
      let cq = db.from("classes").select("id,name,level").eq("status", "ACTIVE").order("name");
      let sq = db.from("subjects").select("id,code,name").eq("status", "ACTIVE").order("name");
      if (teacher) {
        cq = cq.in("id", classIds.length ? classIds : ["00000000-0000-0000-0000-000000000000"]);
        sq = sq.in("id", subjectIds.length ? subjectIds : ["00000000-0000-0000-0000-000000000000"]);
      }
      const [c, s] = await Promise.all([cq, sq]);
      if (c.error) throw c.error; if (s.error) throw s.error;
      setClasses(c.data ?? []); setSubjects(s.data ?? []);
      const r = await db.from("learning_materials").select("id,title,description,material_type,resource_url,class_id,subject_id,teacher_id,status,created_at,classes(name,level),subjects(name,code)").eq("status", "PUBLISHED").order("created_at", { ascending: false });
  ...
  const filtered = useMemo(() => rows.filter(r => {
    const hay = `${text(r.title)} ${text(r.description)} ${text((r.subjects as Row | null)?.name)} ${text((r.classes as Row | null)?.name)}`.toLowerCase();
    return hay.includes(query.toLowerCase().trim()) && (subjectFilter === "ALL" || text(r.subject_id) === subjectFilter);
  ...
  const create = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const r = await db.from("learning_materials").insert({ title: form.title.trim(), description: form.description.trim() || null, material_type: form.material_type, resource_url: form.resource_url.trim(), class_id: form.class_id || null, subject_id: form.subject_id || null, teacher_id: teacherId, status: "PUBLISHED" }).select("id").single();
      if (r.error) throw r.error;
      setForm({ title: "", description: "", material_type: "LINK", resource_url: "", class_id: "", subject_id: "" });
      setShowCreate(false); setMessage("Learning material published successfully."); await load();
  ...
  const openMaterial = (url: string) => {
    try { const parsed = new URL(url); if (!/^https?:$/.test(parsed.protocol)) throw new Error(); window.open(parsed.toString(), "_blank", "noopener,noreferrer"); }
    catch { setMessage("This learning resource has an invalid link."); }
  };
  ...
  return <PortalLayout role={profile.role}><main className="mx-auto w-full max-w-[1440px] space-y-6 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
    ...
    <section className="menwe-card rounded-[1.75rem] p-5 sm:p-7">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center"><label className="relative flex-1"><Search className="absolute left-3 top-3.5 opacity-45" size={17}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search materials, subjects or classes…" className={`${input} pl-10`}/></label><select value={subjectFilter} onChange={e => setSubjectFilter(e.target.value)} className={`${input} lg:w-56`}><option value="ALL">All subjects</option>{subjects.map(s => <option key={text(s.id)} value={text(s.id)}>{text(s.name)}</option>)}</select>{canCreate && <button onClick={() => setShowCreate(v => !v)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--ink)] px-4 text-sm font-bold text-white"><Plus size={17}/>{showCreate ? "Close" : "Add material"}</button>}</div>
      {showCreate && canCreate && <form onSubmit={create} className="mt-5 grid gap-3 rounded-2xl border border-[var(--ink)]/10 bg-[var(--ink)]/[.02] p-4 md:grid-cols-2"><div className="md:col-span-2 flex items-center gap-2"><Sparkles size={17} className="text-[var(--accent)]"/><h2 className="font-semibold">Publish learning material</h2></div><input required maxLength={160} placeholder="Title e.g. Fractions — Introduction" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className={input}/><select required value={form.material_type} onChange={e => setForm({ ...form, material_type: e.target.value })} className={input}><option value="LINK">Web resource</option><option value="VIDEO">Video</option><option value="PDF">PDF</option><option value="DOCUMENT">Document</option><option value="NOTES">Notes</option><option value="OTHER">Other</option></select><select value={form.class_id} onChange={e => setForm({ ...form, class_id: e.target.value })} className={input}><option value="">All eligible classes</option>{classes.map(c => <option key={text(c.id)} value={text(c.id)}>{text(c.name)} · {text(c.level)}</option>)}</select><select value={form.subject_id} onChange={e => setForm({ ...form, subject_id: e.target.value })} className={input}><option value="">General / all subjects</option>{subjects.map(s => <option key={text(s.id)} value={text(s.id)}>{text(s.name)} ({text(s.code)})</option>)}</select><input required type="url" placeholder="https://…" value={form.resource_url} onChange={e => setForm({ ...form, resource_url: e.target.value })} className={`${input} md:col-span-2`}/><textarea maxLength={1200} placeholder="Short learner guidance or description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className={`${input} min-h-24 md:col-span-2`}/><button disabled={busy} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-4 text-sm font-bold text-white md:w-max disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin"/> : <Plus size={16}/>}Publish resource</button></form>}
      {loading ? <div className="grid min-h-48 place-items-center text-sm opacity-60"><Loader2 className="animate-spin" size={22}/></div> : filtered.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--ink)]/15 px-5 py-12 text-center"><BookOpen className="mx-auto opacity-30" size={34}/><h2 className="mt-3 font-semibold">No learning materials yet</h2><p className="mx-auto mt-1 max-w-md text-sm leading-6 opacity-55">{canCreate ? "Publish your first resource above. Keep links focused on the current curriculum and learner needs." : "Your teachers and school administrators have not published learning resources for your account yet."}</p></div> : <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map(r => { const Icon = typeIcon(text(r.material_type)); const subject = r.subjects as Row | null; const cls = r.classes as Row | null; return <article key={text(r.id)} className="group rounded-2xl border border-gray-100 bg-white p-5 shadow-md transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl dark:border-slate-800 dark:bg-slate-950"><div className="flex items-start justify-between gap-3"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-[#061229]/5 text-[#061229] dark:bg-white/8 dark:text-white"><Icon size={21}/></span><span className="rounded-full bg-[#D89B28]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#D89B28]">{text(r.material_type)}</span></div><h3 className="mt-4 text-lg font-semibold text-[#061229] dark:text-white">{text(r.title)}</h3>{r.description && <p className="mt-2 line-clamp-3 text-sm leading-6 opacity-60">{text(r.description)}</p>}<div className="mt-4 flex flex-wrap gap-2 text-[11px] font-semibold"><span className="rounded-full border border-[var(--ink)]/10 px-2.5 py-1">{text(subject?.name, "General")}</span><span className="rounded-full border border-[var(--ink)]/10 px-2.5 py-1">{text(cls?.name, "All eligible classes")}</span></div><button onClick={() => openMaterial(text(r.resource_url))} className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[var(--ink)] px-4 text-sm font-bold text-white transition hover:opacity-90"><ExternalLink size={16}/>Open learning resource</button></article>})}</div>}
    </section>
  </main></PortalLayout>;
}
