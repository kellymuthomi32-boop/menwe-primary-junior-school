import { ArrowRight, ArrowUpRight, Check, ChevronDown, ChevronRight, Facebook, Instagram, Mail, MapPin, Menu, ShieldCheck, Sparkles, X, Youtube, BookOpen, HeartHandshake, GraduationCap } from "lucide-react";
...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  const go = (to: string) => { setLocation(to); setOpen(false); setDiscoverOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return <div className="min-h-screen bg-[var(--paper)] text-[var(--ink)]">
    <header className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? "border-b border-[var(--ink)]/10 bg-[var(--paper)]/92 shadow-[0_10px_35px_rgba(29,43,37,.08)]" : "border-b border-[var(--ink)]/7 bg-[var(--paper)]/82"} backdrop-blur-2xl`}>
      <div className={`hidden overflow-hidden border-b border-[var(--ink)]/[0.06] bg-white/30 transition-all duration-500 lg:block ${scrolled ? "h-0 opacity-0" : "h-8 opacity-100"}`} aria-hidden={scrolled}>
        <div className="mx-auto flex h-8 max-w-7xl items-center justify-between px-8 text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--ink)]/48">
          <span>Menwe Primary & Junior School</span>
          <span className="flex items-center gap-2"><span className="menwe-live-dot" /> A welcoming place to learn</span>
        </div>
      </div>

      <div className={`mx-auto flex max-w-7xl items-center justify-between px-5 transition-all duration-500 lg:px-8 ${scrolled ? "h-[68px]" : "h-[76px]"}`}>
        <button onClick={() => go("/")} className="menwe-focus-ring group shrink-0 rounded-2xl text-left" aria-label="Menwe Primary & Junior School home">
          <SchoolMark />
        </button>

        <nav className="hidden items-center gap-0.5 lg:flex" aria-label="Public navigation">
          <button onClick={() => go("/")} aria-current={location === "/" ? "page" : undefined} className={`relative rounded-full px-3.5 py-2.5 text-[13px] font-semibold transition-all ${location === "/" ? "bg-white text-[var(--ink)] shadow-sm ring-1 ring-[var(--ink)]/8" : "text-[var(--ink)]/62 hover:bg-white/65 hover:text-[var(--ink)]"}`}>
            Home{location === "/" && <span className="absolute inset-x-3 bottom-1 h-0.5 rounded-full bg-[var(--gold)]" />}
          </button>
          {primaryNav.map(([label, path]) => { const active = location === path; return <button key={path} onClick={() => go(path)} aria-current={active ? "page" : undefined} className={`relative rounded-full px-3.5 py-2.5 text-[13px] font-semibold transition-all ${active ? "bg-white text-[var(--ink)] shadow-sm ring-1 ring-[var(--ink)]/8" : "text-[var(--ink)]/62 hover:bg-white/65 hover:text-[var(--ink)]"}`}>{label}{active && <span className="absolute inset-x-3 bottom-1 h-0.5 rounded-full bg-[var(--gold)]" />}</button>; })}
          <div className="relative">
            <button onClick={() => setDiscoverOpen(value => !value)} aria-expanded={discoverOpen} aria-haspopup="menu" className={`flex items-center gap-1.5 rounded-full px-3.5 py-2.5 text-[13px] font-semibold transition-all ${discoverOpen ? "bg-white text-[var(--ink)] shadow-sm ring-1 ring-[var(--ink)]/8" : "text-[var(--ink)]/62 hover:bg-white/65 hover:text-[var(--ink)]"}`}>
              Explore <ChevronDown size={13} className={`transition-transform duration-300 ${discoverOpen ? "rotate-180" : ""}`} />
            </button>
            {discoverOpen && <div role="menu" className="absolute right-0 top-[calc(100%+12px)] w-[310px] overflow-hidden rounded-[1.35rem] border border-[var(--ink)]/9 bg-[var(--paper)]/98 p-2 shadow-[0_24px_60px_rgba(29,43,37,.16)] backdrop-blur-2xl">
              <div className="px-3 pb-2 pt-2"><p className="text-[9px] font-extrabold uppercase tracking-[.22em] text-[var(--accent)]">Discover Menwe</p><p className="mt-1 text-xs leading-5 text-[var(--ink)]/48">A closer look at the people, culture and journey behind the school.</p></div>
              {discoverLinks.map(([label, path]) => <button key={path} role="menuitem" onClick={() => go(path)} className="group flex w-full items-center justify-between rounded-xl px-3 py-3 text-left transition hover:bg-white"><span><span className="block text-sm font-bold">{label}</span><span className="mt-0.5 block text-[10px] text-[var(--ink)]/40">Explore this part of Menwe</span></span><ArrowUpRight size={14} className="text-[var(--ink)]/25 transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-[var(--accent)]" /></button>)}
            </div>}
          </div>
          <button onClick={() => go("/gallery")} aria-current={location === "/gallery" ? "page" : undefined} className={`relative ml-0.5 rounded-full px-3.5 py-2.5 text-[13px] font-semibold transition-all ${location === "/gallery" ? "bg-white text-[var(--ink)] shadow-sm ring-1 ring-[var(--ink)]/8" : "text-[var(--ink)]/62 hover:bg-white/65 hover:text-[var(--ink)]"}`}>Gallery{location === "/gallery" && <span className="absolute inset-x-3 bottom-1 h-0.5 rounded-full bg-[var(--gold)]" />}</button>
          <button onClick={() => go("/contact")} aria-current={location === "/contact" ? "page" : undefined} className={`relative rounded-full px-3.5 py-2.5 text-[13px] font-semibold transition-all ${location === "/contact" ? "bg-white text-[var(--ink)] shadow-sm ring-1 ring-[var(--ink)]/8" : "text-[var(--ink)]/62 hover:bg-white/65 hover:text-[var(--ink)]"}`}>Contact{location === "/contact" && <span className="absolute inset-x-3 bottom-1 h-0.5 rounded-full bg-[var(--gold)]" />}</button>
        </nav>

        <div className="hidden items-center gap-2 lg:flex">
          <button onClick={() => go("/admissions")} className="group inline-flex items-center gap-1.5 rounded-full bg-[var(--ink)] px-4 py-2.5 text-[12px] font-extrabold text-white shadow-[0_8px_24px_rgba(29,43,37,.14)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(29,43,37,.2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
            Start application <ArrowRight size={14} className="transition-transform group-hover:translate-x-0.5" />
          </button>
          <button onClick={() => go("/portal/login")} className="menwe-interactive group flex items-center gap-2 rounded-full border border-[var(--ink)]/12 bg-white/65 px-4 py-2.5 text-[12px] font-bold text-[var(--ink)] shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
            <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--mist)]"><ShieldCheck size={14} className="text-[var(--accent)]" /></span> Portal <ArrowUpRight size={13} className="opacity-40 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </button>
        </div>

        <button onClick={() => setOpen(value => !value)} className="menwe-focus-ring grid h-11 w-11 place-items-center rounded-2xl border border-[var(--ink)]/12 bg-white/75 shadow-sm lg:hidden" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open}>{open ? <X size={20} /> : <Menu size={20} />}</button>
      </div>

      {open && <div className="absolute inset-x-0 top-full max-h-[calc(100vh-68px)] overflow-y-auto border-b border-[var(--ink)]/10 bg-[var(--paper)]/98 px-5 pb-6 pt-3 shadow-[0_24px_40px_rgba(29,43,37,.12)] backdrop-blur-2xl lg:hidden">
        <nav className="mx-auto grid max-w-xl gap-1.5" aria-label="Mobile public navigation">
          {links.map(([label, path]) => { const active = location === path; return <button key={path} onClick={() => go(path)} aria-current={active ? "page" : undefined} className={`flex items-center justify-between rounded-2xl px-4 py-3.5 text-left text-sm font-bold ${active ? "bg-white shadow-sm ring-1 ring-[var(--ink)]/8" : "hover:bg-white/70"}`}><span>{label}</span>{active && <Check size={17} className="text-[var(--accent)]" />}</button>; })}
          <div className="my-1 grid grid-cols-3 gap-1.5">
            {discoverLinks.map(([label, path]) => <button key={path} onClick={() => go(path)} className="rounded-2xl border border-[var(--ink)]/8 bg-white/55 px-3 py-3 text-left transition hover:bg-white"><span className="block text-[10px] font-extrabold uppercase tracking-[.12em] text-[var(--accent)]">Explore</span><span className="mt-1 block text-xs font-bold">{label}</span></button>)}
          </div>
          <button onClick={() => go("/admissions")} className="mt-1 flex items-center justify-center gap-2 rounded-2xl bg-[var(--ink)] px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-[var(--ink)]/15"><ArrowRight size={17} /> Start an application</button>
          <button onClick={() => go("/portal/login")} className="flex items-center justify-center gap-2 rounded-2xl border border-[var(--ink)]/10 bg-white px-4 py-3.5 text-sm font-bold text-[var(--ink)]"><ShieldCheck size={17} className="text-[var(--accent)]" /> Secure portal login <ArrowUpRight size={15} className="opacity-60" /></button>
        </nav>
      </div>}
    </header>
    <main>{children}</main>
    <footer className="relative mt-24 overflow-hidden bg-[var(--ink)] text-white">
      ...
      <div className="relative mx-auto max-w-[1440px] px-5 pb-8 pt-14 sm:pt-16 lg:px-10 lg:pt-20">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.15fr_.75fr_.75fr_1fr]">
          ...
          <div><p className="text-[9px] font-extrabold uppercase tracking-[.24em] text-[var(--gold)]">Connect</p><div className="mt-5 rounded-[1.35rem] border border-white/9 bg-white/[.045] p-4 backdrop-blur-xl"><p className="text-sm font-bold text-white">Community channels</p><p className="mt-1 text-xs leading-5 text-white/40">Official social links can be configured for the school without changing the public layout.</p><div className="mt-4 grid grid-cols-3 gap-2">{socialChannels.map(({ label, icon: Icon, env }) => { const href = import.meta.env[env]; return href ? <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={`Menwe ${label}`} className="menwe-interactive flex h-10 items-center justify-center rounded-xl border border-white/10 bg-white/[.06] text-white/65 hover:border-[var(--gold)]/25 hover:text-white"><Icon size={17} /></a> : <button key={label} type="button" onClick={() => go("/contact")} title={`${label} link is managed by the school`} aria-label={`${label} — contact the school`} className="menwe-interactive flex h-10 items-center justify-center rounded-xl border border-white/10 bg-white/[.035] text-white/30 hover:border-white/15 hover:text-white/60"><Icon size={17} /></button>; })}</div><button onClick={() => go("/contact")} className="mt-3 flex w-full items-center justify-between rounded-xl border border-white/8 bg-white/[.035] px-3 py-2.5 text-left text-xs font-semibold text-white/55 transition hover:bg-white/[.07] hover:text-white"><span className="flex items-center gap-2"><Mail size={14} /> Contact the school</span><ArrowUpRight size={13} /></button></div></div>
        </div>
