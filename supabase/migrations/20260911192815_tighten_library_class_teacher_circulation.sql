...
begin
  ...
  insert into public.library_replacement_records(loan_id,book_id,learner_id,replacement_type,expected_cost,notes) select v_loan.id,v_loan.book_id,v_loan.student_id,'BUY_NEW',coalesce(v_book.replacement_cost,0),coalesce(p_notes,'Lost textbook; new replacement required.') where not exists(select 1 from public.library_replacement_records where loan_id=v_loan.id and resolved_at is null);
  return v_loan;
end; $$;
