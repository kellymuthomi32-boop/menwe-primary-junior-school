  ...
  select exists (
    ...
    where e.student_id = p_student_id
      ...
      and (
        ...
        or exists (
          select 1 from public.teacher_class_roles tcr
          where tcr.class_id = c.id and tcr.teacher_id = private.current_teacher_id() and tcr.is_class_teacher = true
        )
      )
  );
$$;
...
returns public.library_loans language plpgsql set search_path='public' as $$
...
begin
  ...
  insert into public.library_loans(book_id,student_id,issued_by,issued_at,due_at,issue_date,return_date,status,replacement_required,replacement_received,notes) values(p_book_id,p_student_id,auth.uid(),now(),p_due_at,p_issue_date,p_due_at::date,'ISSUED',false,false,p_notes) returning * into v_loan;
...
create policy "library loans managed by authorised staff" on public.library_loans for all to authenticated using ((select private.is_admin()) or (select private.is_library_class_teacher(student_id))) with check ((select private.is_admin()) or (select private.is_library_class_teacher(student_id)));
drop policy if exists "library loans visible to borrower or staff" on public.library_loans;
create policy "library loans visible to borrower or authorised staff" on public.library_loans for select to authenticated using (exists(select 1 from public.students s where s.id=library_loans.student_id and s.profile_id=(select auth.uid())) or exists(select 1 from public.student_parents sp join public.parents pa on pa.id=sp.parent_id where sp.student_id=library_loans.student_id and pa.profile_id=(select auth.uid())) or (select private.is_admin()) or (select private.is_library_class_teacher(student_id)));
drop policy if exists "library_replacements_staff" on public.library_replacement_records;
create policy "library replacements readable by staff" on public.library_replacement_records for select to authenticated using ((select private.is_admin()) or (select private.is_library_class_teacher(learner_id)));
create policy "library replacements managed by administrators" on public.library_replacement_records for insert to authenticated with check ((select private.is_admin()));
create policy "library replacements updated by administrators" on public.library_replacement_records for update to authenticated using ((select private.is_admin())) with check ((select private.is_admin()));
create policy "library replacements deleted by administrators" on public.library_replacement_records for delete to authenticated using ((select private.is_admin()));
