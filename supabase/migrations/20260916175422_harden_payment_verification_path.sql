create or replace function public.guard_payment_verification_path()
returns trigger
language plpgsql
...
begin
  if old.status = 'VERIFIED' then
    if new.status <> old.status
      ...
      raise exception 'A verified payment is immutable.' using errcode = '42501';
    end if;
  end if;

  if new.status = 'VERIFIED' and old.status <> 'VERIFIED'
     and current_setting('menwe.payment_verification', true) <> '1' then
    raise exception 'Payments must be verified through the controlled verification workflow.' using errcode = '42501';
  end if;
...
drop trigger if exists payments_guard_verification on public.payments;
create trigger payments_guard_verification
before update on public.payments
for each row execute function public.guard_payment_verification_path();

create or replace function public.verify_payment(p_payment_id uuid, p_provider_reference text, p_event_payload jsonb default '{}'::jsonb)
returns void
language plpgsql
security invoker
...
begin
  ...
  if v_payment.status = 'VERIFIED' then
    if v_payment.provider_reference = trim(p_provider_reference) then
      return;
    end if;
  ...
  perform set_config('menwe.payment_verification', '1', true);
  ...
  set status = 'VERIFIED',
      ...
      metadata = coalesce(p_event_payload, '{}'::jsonb)
  ...
  from public.invoice_items where invoice_id = v_payment.invoice_id;
  select coalesce(sum(amount), 0) into v_verified
  from public.payments where invoice_id = v_payment.invoice_id and status = 'VERIFIED';

  update public.invoices
  set status = case
    when v_verified >= v_due then 'PAID'::public.invoice_status
    when v_verified > 0 then 'PARTIALLY_PAID'::public.invoice_status
    else status
  end
